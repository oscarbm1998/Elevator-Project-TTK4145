...
func requests_above(Elevator e) (int returnval){
...
func requests_below(Elevator e) (int returnval){
    for f = 0 f < e.floor f++{
        for btn = 0 btn < N_BUTTONS btn++{
            if e.requests[f][btn]{
                return 1
...
func requests_here(Elevator e) (int returnval){
    for btn = 0; btn < N_BUTTONS; btn++{
...
func requests_nextAction(Elevator e) (Action){
    switch(e.dirn){
    case D_Up:
